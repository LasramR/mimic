...
def hook_action(project_dir : str, hook_config : MimicHookConfig, variables: Dict[str, Any], unsafe_mode : bool = False) -> bool :
  for command in hook_config.steps:
    parsed_command = inject_variable(command, variables)
    if unsafe_mode or get_user_confirmation(f"{project_dir}: `{parsed_command}` will be executed. Continue Y/n ?").upper() == "Y":
      command_cp = Popen([parsed_command], cwd=project_dir, text=True, shell=True, stdout=PIPE, stderr=PIPE);
